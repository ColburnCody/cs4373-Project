...
async function deleteProduct(docId, context) {
    ...
    try {
        await admin.firestore().collection(Constant.collectionNames.PRODUCT).doc(docId).delete();
    } catch (e) {
        if (Constant.DEV) console.log(e);
...
async function updateProduct(productInfo, context) {
    ...
    try {
        await admin.firestore().collection(Constant.collectionNames.PRODUCT).doc(productInfo.docId).update(productInfo.data);
    } catch (e) {
        if (Constant.DEV) console.log(e);
...
async function getProductById(data, context) {
    ...
    try {
        const doc = await admin.firestore().collection(Constant.collectionNames.PRODUCT).doc(data).get();
        if (doc.exists) {
